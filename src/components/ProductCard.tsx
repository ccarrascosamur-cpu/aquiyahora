...
export function ProductCard({ product, isAdmin, onEdit, index }: ProductCardProps) {
  ...
  return (
    <motion.div
      ...
      className="group relative bg-white rounded-card border border-border-custom overflow-hidden hover:-translate-y-1 hover:shadow-card-hover transition-all duration-250 flex flex-col"
    >
      {/* Image area */}
      ...
      <div className="p-4 md:p-5 flex flex-col flex-1">
        ...
        {product.aromas.length > 0 && product.aromas[0] !== '' && (
          <p className="font-body text-xs text-text-muted mt-3 pt-3 border-t border-border-custom/50 line-clamp-2">
            <span className="text-accent-rose font-medium">Aromas: </span>
            {product.aromas.join(', ')}
          </p>
        )}
      </div>
